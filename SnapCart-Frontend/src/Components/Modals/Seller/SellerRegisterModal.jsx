...
const SellerRegisterModal = ({ isOpen, onClose, switchToLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const checkEmailRes = await axios.get(
        `${API_BASE_URL}/sellers/check-email?email=${formData.email}`,
        {
          headers: {
      ...
      if (checkEmailRes.data?.data?.exists) {
        ...
        setErrors((prev) => ({
          ...prev,
          email: "This email is already registered as a seller",
        }));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70">
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-md px-10 py-12">
        {/* Header */}
