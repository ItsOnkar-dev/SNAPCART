...
const BecomeSeller = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const url = seller 
        ? `http://localhost:8000/sellers/${seller.id}` 
        : "http://localhost:8000/sellers";
      
      const method = seller ? axios.put : axios.post;
      await method(url, formData);
      console.log(`Seller ${seller ? 'updated' : 'created'} successfully`);
  ...
  const styles = {
    input: "shadow appearance-none border dark:border-gray-500 rounded w-full py-3 px-4 text-gray-700 dark:text-white/80 dark:bg-slate-800 leading-tight focus:outline-none focus:border-transparent transition-all duration-300",
    ...
    paragraph: "text-gray-600 dark:text-slate-800",
    formLabel: "block text-gray-700 dark:text-white/80 text-sm font-bold mb-2",
    errorText: "text-red-500 text-sm mt-1",
    submitButton: "w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-lg font-medium text-white bg-indigo-500"
  };
  ...
  const FormInput = ({ id, label, type = "text", placeholder, value, error }) => (
    <div>
      ...
      <input
        ...
        className={`${styles.input} ${error ? 'border-red-500 bg-red-50' : 'border-gray-300'}`}
      />
        ...
        <div className="bg-white dark:bg-slate-900 border dark:border-gray-600 shadow-2xl rounded-lg overflow-hidden">
          ...
          <div className="p-6 md:p-12">
            ...
            {contextErrors && contextErrors.submit && (
              <ErrorAlert message={contextErrors.submit} />
            )}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    rows="5"
                    className={`${styles.input} ${errors.storeDescription ? 'border-red-500 bg-red-50' : 'border-gray-300'}`}
                  />
                ...
                <button
                  ...
                  className={`${styles.submitButton} ${isSubmitting ? 'opacity-75 cursor-not-allowed' : ''}`}
                >
                  {isSubmitting ? (
                    <>
